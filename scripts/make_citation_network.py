...
def write_cytoscape_html(out, G_plot, author_lookup, partition, pos,
    ...
    top_n: if set, restrict the graph to the top-N nodes by in-degree before
           building the HTML (reduces file size and load time).
    """
    ...
    if top_n and len(node_ids) > top_n:
        node_ids  = node_ids[:top_n]
        keep      = set(node_ids)
        G_plot    = G_plot.subgraph(keep).copy()
        indegree_map = dict(G_plot.in_degree())
        edge_papers_map = {k: v for k, v in edge_papers_map.items()
...
"""
    ...
    cy_file = os.path.join(out, _pf(output_filename))
...
"""
    ...
    _shared = dict(
        author_lookup=author_lookup, partition=partition, pos=pos,
        pagerank=pagerank, edge_papers_map=edge_papers_map,
    ...
    write_cytoscape_html(
        out, G_plot, top_n=200,
        output_filename="citation_network_cytoscape.html",
        **_shared,
    )

    print("Writing full citation network …")
    write_cytoscape_html(
        out, G_plot,
        output_filename="citation_network_cytoscape_full.html",
        **_shared,
    )
